...
public class ArchiveEntitiyLazyModel extends LazyDataModel<ArchiveGDTO> {
	...
	public List<ArchiveGDTO> load(int first, int pageSize, String sortField, SortOrder sortOrder,
		...
		archives = getArchiveService().getArchives(first, pageSize, sortField, sortOrder, filtermap);
		...
		for (ArchiveGDTO arc : archives) {
			...
			if (filters != null ) {
				...
				for (Entry<String, Object> filter : filters.entrySet()) {			
					...
					switch (key) {
					case "archiveId":
						if (value != null && !arc.getArchiveId().contains(value.toString()))
							match = false;
						break;
					case "receiver":
						if (value != null && !arc.getReceiver().contains(value.toString()))
							match = false;
						break;						
					default:
						break;
